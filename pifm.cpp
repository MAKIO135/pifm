            ...
            next( next ) {
                for(int i = 0; i < QUALITY; i ++ ) {  // sample
                    for(int j = 0; j < SQUALITY; j ++ ) { // starttime
                        ...
                        if( x == 0 ) sincLUT[ j ][ i ] = 1.0; // sin(0)/(0) == 1, says my limits therory
...
void playWav( char* filename, float samplerate, bool stereo ) {
    ...
    if( stereo ) {
        ...
        ss = new StereoSplitter(
            new PreEmp( samplerate, new Resamp( samplerate, 152000, sm->getChannel( 0 ) ) ), // left
            new PreEmp( samplerate, new Resamp( samplerate, 152000, sm->getChannel( 1 ) ) ) // Right
        );
    } else {
        ss = new Mono( new PreEmp( samplerate, new Outputter( samplerate ) ) );
    }

    for( int i = 0; i < 22; i++ ) read( fp, &data, 2 ); // read past header
